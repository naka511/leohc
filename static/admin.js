document.addEventListener("DOMContentLoaded", async () => {
  ...
  function renderTable(tokens, summary = null, pagination = null) {
    ...
    pageTokens.forEach(t => {
      ...
      const statusClass = tokenExpired ? "status-invalid" : `status-${t.status.toLowerCase()}`;
      ...
      const displayStatus = tokenExpired ? "\u5df2\u5931\u6548" : (STATUS_MAP[t.status.toLowerCase()] || t.status);
